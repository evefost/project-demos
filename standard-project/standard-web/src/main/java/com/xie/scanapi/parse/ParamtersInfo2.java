...
import com.xie.scanapi.Class2JsonUtils2;
 ...
 * 参数信息
 */
public class ParamtersInfo2 implements IInfo {
    ...
    public ParamtersInfo2(ResolverSupport resolverSupport, Type rawType) {
    ...
    public ParamtersInfo2(ResolverSupport resolverSupport, Type rawType, Type[] actualTypeArguments) {
        this.resolverSupport = resolverSupport;
        this.rawType = rawType;
        ...
        if (isGener) {
            ...
            for (int i = 0; i < actualTypeArguments.length; i++) {
                ...
                if (ar instanceof ParameterizedTypeImpl) {
                    ...
                    ParamtersInfo2 pInfo = new ParamtersInfo2(resolverSupport, tar.getRawType(), tar.getActualTypeArguments());
                   return pInfo.parse();
                } else {
            ...
            sb.append("``` \n");
            StringBuffer rs = Class2JsonUtils2.generateApiJsonForm((Class) rawType, false, true, actualTypeMap);
            sb.append(rs);
            sb.append("\n``` ");
            return sb;
        } else {
            ...
            if (ClassHelper.isBaseClass(clz)) {
                ...
                StringBuffer sb = new StringBuffer();
                sb.append("``` \n");
                StringBuffer jsonDes = Class2JsonUtils2.generateApiJsonForm((Class) rawType, false, true);
                sb.append(jsonDes);
                sb.append("\n``` ");
                return sb;
            }
